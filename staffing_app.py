...
# Import from models
from db import db
from db.models import User, Client, Consultant, Job, Application, Placement, Skill, JobSkill
...
def unified_sync_status():
    ...
    try:
        from unified_sync_manager import get_unified_sync_status
        return jsonify(get_unified_sync_status())
    except ImportError:
        return jsonify({
            'status': 'service_unavailable',
            'message': 'Unified sync manager not configured',
            'platforms': {
        ...
        logger.error(f"Error getting unified sync status: {str(e)}")
        return jsonify({'error': 'Unified sync status unavailable'})
...
def force_unified_sync():
    ...
    try:
        from unified_sync_manager import force_unified_sync
        result = force_unified_sync()
...
if __name__ == '__main__':
    # Start 24/7 sync monitoring service
    from replit_sync_service import start_sync_monitoring
    start_sync_monitoring()
    logger.info("24/7 sync monitoring active: Replit ↔ GitHub ↔ Azure")
